...
export default function AchievementsClient({ achievements }) {
    useEffect(() => {
        ...
        const items = document.querySelectorAll('.achievement-gallery-item, .fade-in');
    ...
    const gridHTML = achievements
        .map(
            ({ src, caption }, i) => `
      <div class="achievement-gallery-item" style="transition-delay: ${i * 0.08}s;">
        <img src="${src}" alt="${caption || `Achievement ${i + 1}`}" loading="lazy">
        ${caption ? `<div class="achievement-caption"><p>${caption}</p></div>` : ''}
      </div>`
        )
        .join('');
    ...
    const emptyState = achievements.length === 0
        ? `<div class="achievements-empty">
            <p>Achievement photos coming soon! Add images to <code>public/images/achievements/</code> and they will appear here automatically.</p>
           </div>`
        : '';
    ...
    <section class="sub-page-hero">
        ...
        <p>Celebrating the milestones, awards, and recognition that mark our journey of craftsmanship and artistry.</p>
    </section>
